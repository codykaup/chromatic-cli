...
run "Edit .storybook/preview.ts"            "packages/$PKG/.storybook/preview.ts"            "0 stories; <storybookConfig> AND the preview entry both move (covered twice on purpose)"
run "Edit .storybook/main.ts"               "packages/$PKG/.storybook/main.ts"               "0 stories; <storybookConfig> moves (hashed off disk, never a module)"
run_nm "Edit dependency react/index.js"     "react/index.js"                                 "all 3 on webpack; <storybookGlobals> on vite (esbuild elides the type-only import — decided safe)"
run_nm "Edit dependency moment"             "moment/dist/moment.js"                          "Button on vite + webpack; <storybookGlobals> on rspack (0 stories indexed)"
echo "========================================================"
