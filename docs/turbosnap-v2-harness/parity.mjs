#!/usr/bin/env node
// parity.mjs <base-manifest> <cur-manifest> <v1-result.json>
...
const [baseFile, curFile, v1File] = process.argv.slice(2);
if (!baseFile || !curFile || !v1File) {
  console.error('usage: node parity.mjs <base.json> <cur.json> <v1-result.json>');
  process.exit(2);
}
...
const STORY_FILE = /\.stories\.m?[jt]sx?$/;
...
const v1Reported = v1.status === 'bailed' ? [] : (v1.storyFiles ?? []);
...
if (missing.length === 0 && extra.length === 0) {
  ...
  verdict = 'v2 wider (over-captures — safe)';
} else if (a.blunt) {
