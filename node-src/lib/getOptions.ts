...
export default function getOptions(
  ...
  buildScriptName = findBuildScriptName(packageJson.scripts as Scripts, buildScriptName);
 ...
 *
 * Exported so `chromatic turbosnap-manifest` picks the same script a real build would, and therefore
 * reads the same `-c`/`-s` flags out of it.
 *
 ...
 */
export function findBuildScriptName(scripts: Scripts = {}, buildScriptName?: string) {
  if (typeof buildScriptName === 'string') return buildScriptName;

  if (scripts['build-storybook']) return 'build-storybook';

  const [key] =
    Object.entries(scripts).find(([, script]) => script.startsWith('build-storybook')) || [];
