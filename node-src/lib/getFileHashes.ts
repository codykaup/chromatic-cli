  ...
  const hashes = await Promise.all(
    files.map((file) =>
      limit(async () => {
        ...
        // memory is bounded by `concurrency` rather than by the number of files. Allocating up front
        // means a large static asset directory reserves gigabytes before the first read.
