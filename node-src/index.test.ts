...
describe('shouldUploadMetadata', () => {
  ...
  it('does not upload for an unpublished build, even if --upload-metadata is enabled', () => {
    // A build that never got published (e.g. `prepare` failed) only has a baseline build without a
    // storybookUrl, so there's no valid location to host the metadata files.
