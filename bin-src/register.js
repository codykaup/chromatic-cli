...
const commands = {
  ...
  'hash-manifest': () =>
    import('./hashManifest').then(({ main: hashManifestMain }) =>
      hashManifestMain(process.argv.slice(3))
    ),
  'hash-diff': () =>
    import('./hashDiff').then(({ main: hashDiffMain }) => hashDiffMain(process.argv.slice(3))),
